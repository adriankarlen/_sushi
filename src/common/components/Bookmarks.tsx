...
export const Bookmarks = () => {
  const rounded = (index: number, length: number) => {
    if (index === 0) return "first-of-type:rounded-tl-2xl last-of-type:rounded-bl-2xl";
    if (index === length - 1) return "first-of-type:rounded-tr-2xl last-of-type:rounded-br-2xl";
    return "";
  };
  ...
  return (
    <div id="bookmarks" className="flex flex-row items-start justify-around md:flex-row">
      ...
      {bookmarks.map((category, index) => (
        <ul key={category.description} className="flex flex-col justify-around w-full pr-1 last-of-type:pr-0">
